...
def getownteam(hexxx):
    ...
    data = json.loads(a)
    return data['5']['1']
def xoakbbot(jwt_token, player_id):
    url = f"http://160.250.247.175:5014/remove_friend?token={jwt_token}&id={player_id}"
    ...
    if res.status_code == 200:
        ...
        return None
def add_uid_to_file(player_id, duration_days=30):
    file_path = "uids.json"
    ...
    if os.path.exists(file_path):
        ...
        data = {}
    data[player_id] = expiration_time.strftime("%Y-%m-%d %H:%M:%S")
    with open(file_path, "w") as f:
        json.dump(data, f, indent=4)
    print(f"Added UID {player_id} with expiration {expiration_time}")
def check_and_remove_expired_uids(jwt_token):
    file_path = "uids.json"
    current_time = datetime.now()
    ...
    with open(file_path, "r") as f:
        try:
            ...
            return
    expired_uids = []
    for player_id, expiration_str in list(data.items()):
        expiration_time = datetime.strptime(expiration_str, "%Y-%m-%d %H:%M:%S")
        if current_time >= expiration_time:
            result = xoakbbot(jwt_token, player_id)
            if result:
                expired_uids.append(player_id)
    for player_id in expired_uids:
        data.pop(player_id, None)
        print(f"Removed expired UID {player_id}")
    with open(file_path, "w") as f:
        json.dump(data, f, indent=4)
...
def send_likes(uid):
    try:
        likes_api_response = requests.get(
            f"http://160.250.137.144:5001/like?uid={uid}&server_name=sg&key=casang207"
        )
...
[C][B][11EAFD]‎━━━━━━━━━━━━
[C][B][FFB300]Tạo Bởi: [FFFFFF]Cái Hoàng [00FF00]Luân!!
                """
            else:
                message = (
...
def get_random_avatar():
    avatar_list = [
        '902000011', '902000018', '902000029', '902000024', '902000065', 
        '902000061', '902000060', '902000077', '902000087', '902000094', 
        '902000132', '902000192', '902000207', '902000208', '902000210','902000209','902000211'
    ]
    return random.choice(avatar_list)

def get_jwt_token():
    global jwt_token
    url = "https://amdts-likefreefire.vercel.app/api/get_jwt?guest_uid=4061139812&guest_password=F36A4F625669F777402EF8F5B4E64C25E3613D78C4635AABC6D16B7A9B39728B"
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Lấy token trực tiếp từ phản hồi
            if "token" in data:
...
[00FFFF]Contact me on Te lé gờ ràm: [FF1493]@tranbaodev
[C][B][11EAFD]‎━━━━━━━━━━━━
[C][B][FFB300]Credits: Con Bò!!
        """
        return time
...
def spam_requests(player_id):
        url = f"https://spam-kb-ayavcl.onrender.com/spam?id={player_id}"
        res = requests.get(url)
...
‎━━━━━━━━━━━━
[808080][B][C]Tạo Bởi: [FFFFFF]Cái Hoàng [00FF00]Luân!!
    """
        return msg_visit
    else:
...
[00FF00]Teleown: @caihoangluan

[FFFFFF]Full Lệnh Gửi :
[00FF00]/🗿help
[C][B][00FFFF]━━━━━━━━━━━━
[C][B][808080]Tạo Bởi: [FFFFFF]Cái Hoàng [00FF00]Luân!!
""",uid
                            )
                        )
                else:
...
[808080][b][c]Tạo Bởi: [FFFFFF]Cái Hoàng [00FF00]Luân!!
            """,uid))
                    ...
                    try:
                        ...
                        restart_program()
            if "1200" in data.hex()[0:4] and b"/xoakb" in data:
                try:
                    ...
                    parsed_data = json.loads(json_result)
                    uid = parsed_data["5"]["data"]["1"]["data"]
                    command_split = re.split(r"/xoakb", str(data))
                    player_id = command_split[1].split('(')[0].strip()             
                    with open("token.txt", "r") as f:
                        jwt_token = f.read().strip()
                    xoakbbot(jwt_token, player_id)
                except Exception as e:
                    pass
            if "1200" in data.hex()[0:4] and b"/kb" in data:
                try:
                    ...
                    parsed_data = json.loads(json_result)
                    uid = parsed_data["5"]["data"]["1"]["data"]
                    command_split = re.split(r"/kb", str(data))
                    player_id = command_split[1].split('(')[0].strip()   
                    with open("token.txt", "r") as f:
                        jwt_token = f.read().strip()
                    tmr.RequestAddingFriend(3975170787, player_id, jwt_token)
                    add_uid_to_file(player_id, duration_days=30)
                    clients.send(self.GenResponsMsg("[FFFFFF]Bot Đã G-ửi L-ời Mời Kết Bạn.", uid))
                    check_and_remove_expired_uids(jwt_token)
                except Exception as e:
                    pass
...
[808080][b][c]Tạo Bởi: [FFFFFF]Cái Hoàng [00FF00]Luân!!
            ...
            if "1200" in data.hex()[0:4] and b"/ban/" in data:
                try:
                    ...
                    parsed_data = json.loads(json_result)
                    uid = parsed_data["5"]["data"]["1"]["data"]
                    clients.send(self.GenResponsMsg("Vâng tôi hiểu rồi, xin hãy đợi..", uid))
...
[FFFFFF][B]Tạo Team 3 Đến Team 6:
[00FF00]/🗿3s  
[00FF00]/🗿4s  
[00FF00]/🗿5s  
[00FF00]/🗿6s  
...
[FFFFFF]Bot Giá Hiện Tại Là 60k Tháng TikTok Tranbaodev
...
if __name__ == "__main__":
    try:
        client_thread = FF_CLIENT(id="3975170787", password="F2EA1CF2E2E08EA97D4A769363DA2BE12445272D9CBDC433777500CD48ADA1F5")
